...
class SingleCorrectQuestions extends StatelessWidget {
  final String questionText;
  final String description;
  final double marks;
  final double negativeMark;
  final Map<String, String> choices; // Change from List<String> to Map<String, String>
  final String correctAnswer; // Changed to store correct answer key (e.g., "A")
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Single Correct Questions', style: GoogleFonts.poppins(color: Colors.white)),
      ...
      body: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Row(children: [
                Expanded(child: _buildCard('Question:', questionText))
              ]),
              _buildOptions(),
              Row(children: [
                Expanded(child: _buildCard('Description:', description))
              ]),
              _buildMarksInfo(),
  ...
  Widget _buildOptions() {
    return Column(
      children: choices.entries.map((entry) {
        final index = choices.keys.toList().indexOf(entry.key);
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 8.0),
          child: Card(
            elevation: 4,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
            color: correctAnswer == entry.key ? Colors.green.shade100 : Colors.blue.shade100,
            child: ListTile(
              leading: CircleAvatar(
                backgroundColor: correctAnswer == entry.key ? Colors.green : Colors.blue,
                child: Text(
              ...
              title: Text(
                ...
                style: GoogleFonts.poppins(
                  color: correctAnswer == entry.key ? Colors.green.shade700 : Colors.blue.shade700,
                ),
              ),
            ),
          ),
